...
with tab0:
    ...
    col_norm1.metric(f"Statistik Test ({test_name})", f"{stat_val:.4f}")
    col_norm2.metric("p-value", f"{p_val:.4e}")

    if p_val < 0.05:
        st.info(
            "ℹ️ **Kesimpulan Normalitas:** Data `berat_kg` **tidak terdistribusi normal** (p-value < 0.05). "
            "Kondisi ini wajar untuk data hasil tangkapan perikanan dan mendukung penggunaan pemodelan GLM/GAM (Poisson, Negative Binomial, Tweedie)."
        )
    else:
        st.info(
            "ℹ️ **Kesimpulan Normalitas:** Data `berat_kg` **terdistribusi normal** (p-value >= 0.05)."
        )
    ...
    for cat in valid_cats:
        ...
        if len(groups) > 1:
            ...
            het_results.append({
                ...
                "Jumlah Kategori": len(groups),
                "Statistik Levene": round(stat_l, 4),
                "p-value": f"{p_l:.4e}",
                "Status Varians": (
                    "Heterogen (p < 0.05)"
                    if p_l < 0.05
    ...
    if het_results:
        ...
        # Mengatur lebar tabel agar tidak terlalu lebar menggunakan kolom Streamlit & use_container_width=False
    ...
    try:
        ...
        # Mengatur lebar tabel agar fit dengan karakter isi
